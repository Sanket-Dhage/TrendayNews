        ...
        {news.map((item, index) => (
          ...
          >
            {index < 6 && item.multimedia?.length > 0 && (
              <div
                className="h-60 bg-gray-300 bg-cover bg-center rounded-t-lg"
            ...
            <div className="py-2 flex flex-col gap-2">
              ...
              <h2 className="text-xl font-bold px-4">{item.title}</h2>
              <p className="text-sm text-gray-500 px-4">{item.abstract}</p>
            </div>
          </Link>
        ))}
      </div>
